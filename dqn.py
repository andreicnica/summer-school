...
class DQN(nn.Module):
    ...
    def __init__(self, in_size: torch.Size, out_size: torch.Size):
        ...
        self.conv1 = nn.Conv2d(in_size[0], 16, kernel_size=5, stride=1)
...
def eval_agent(ep_no):
    ...
    for _ in range(eval_episodes):
        ep_return = 0
        state = env.reset().unsqueeze(0).unsqueeze(0)
        for t in count():
            action = select_action(state.to(device), eval=True)
            screen, reward, done, _ = env.step(action.item())
            state = screen.unsqueeze(0).unsqueeze(0)
            ep_return += reward
            if done:
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if done:
            ...
            ep_return = 0
            if i_episode % 20 == 19:
                print(f"[Train {i_episode:d}] Return:", np.mean(returns), " | Ep. length:", np.mean(episode_durations))
